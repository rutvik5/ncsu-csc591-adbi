...
def records_to_graph():
    ...
    for genre in genres:
    	genre_node = Node(id= id_gnode, name= genre, type= 'genre')
    	nodedict[genre] = genre_node
    	nodelist.append(genre_node)
    	id_gnode = id_gnode + 1

    for rating in ratings:
    	rating_node = Node(id= id_gnode, name= rating, type= 'user')
    	nodedict[rating] = rating_node
    	nodelist.append(rating_node)
    	id_gnode = id_gnode + 1

    for director in directors:
    	director_node = Node(id= id_gnode, name= director, type= 'director')
    	nodedict[director] = director_node
    	nodelist.append(director_node)
    	id_gnode = id_gnode + 1

    for actor in actors:
    	actor_node = Node(id= id_gnode, name= actor, type= 'actor')
    	nodedict[actor] = actor_node
    	nodelist.append(actor_node)
    	id_gnode = id_gnode + 1

    for movie in movies:
    	movie_node = Node(id= id_gnode, name= movie, type= 'movie')
    	nodedict[movie] = movie_node
    	nodelist.append(movie_node)
    	id_gnode  = id_gnode + 1
    	
    	for rating in rating_all:
    		movie_node = Node(id= id_gnode, name= movie + "_" + rating, type= 'movie-rating')
    		nodedict[movie + "_" + rating] = movie_node
    		nodelist.append(movie_node)
    		id_gnode = id_gnode + 1
    ...
    for rating in ratings:
    	node = nodedict[rating]
    	curr_rating = ratings[rating]

    	for m_rating in curr_rating:
    		rating_node = m_rating + "_" + curr_rating[m_rating]
    		m_r_node = nodedict[rating_node]
    		node.neighbors.append(m_r_node)
    		m_r_node.neighbors.append(node)

    for movie in movies:

    	movie_node = nodedict[movie]
    	curr_movie = movies[movie]

    	if (curr_movie.director != None):
        director_node = nodedict[curr_movie.director]
        movie_node.neighbors.append(director_node)
        director_node.neighbors.append(movie_node)

      curr_actors = curr_movie.actors
      ...
      curr_genres = curr_movie.genres
    	...
    	for rating in rating_all:
    		movie_rating = movie + "_" + rating
    		node = nodedict[movie_rating]
    		node.neighbors.append(movie_node)
    		movie_node.neighbors.append(node)
